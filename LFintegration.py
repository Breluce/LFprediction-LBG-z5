...
#%%
#run integration on survey with published LBG count predictions (to gauge efficacy of our calculation)

#IWATA 2003
#SUB_skyarea = (836*u.arcmin**2).to(u.deg**2)

# surveys = pd.DataFrame(columns = ['survey', 'survey_area', 'imag_lim', 'ONOcc_counts','int_counts', 'int_err']) 

# surveys.survey = ['DES0332-2749','UD-SXDS', 'UD-COSMOS', 'D-XMM-LSS', 'D-COSMOS', 'D-ELAIS-N1', 'D-DEEP2-3', 'W-XMM', 'W-GAMA09H', 'W-WIDE12H', 'W-GAMA15H', 'W-HECTOMAP', 'W-VVDS']
# survey_area = [0.534, 1.1, 1.3, 2.4, 6.5, 3.3, 5.5, 28.5, 12.4, 15.2, 16.6, 4.8, 5.1]
# surveys.survey_area = [Quantity(x, "deg2") for x in survey_area]
# surveys.imag_lim = [24.0, 26.53, 26.46, 25.88, 26.04, 25.87, 25.96, 25.71, 25.65, 25.82, 25.81, 25.82, 25.74]
# surveys.ONOcc_counts = [np.nan, 1209, 1990, 711, 6282, 612, 1498, 6371, 5989, 5243, 6457, 1082, 1500]

# counts = []
# errors = []

# for i in tqdm(range(len(surveys))):
#     ct, err = count_pred(surveys.survey_area[i], surveys.imag_lim[i], z5_params, zgrid)
#     counts.append(ct)
#     errors.append(err)

# surveys.int_counts = counts
# surveys.int_err = errors
...
# ax3.bar(ind, (surveys.ONOcc_counts[1:13]), width, color = 'blue', label = 'Ono 2018 color-selection \n z-band S/N > 5 \n g-band < 2σ')
# ax3.bar(ind+width, (surveys.int_counts[1:13]), width, color = 'tan', label = 'Count pred from integration')
# #ax3.bar(ind+width, (surveys.int_counts[1:13]), width, yerr=surveys.int_err[1:13], color = 'orange', align='center', alpha=0.5, ecolor='black', capsize=10)
# plt.xticks(ind+width,surveys.survey[1:13])
